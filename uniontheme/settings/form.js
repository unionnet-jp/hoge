;(() => {
  const formsEls = [...document.querySelectorAll('form.wpcf7-form')]
  formsEls.forEach((formEl) => {
    const formButton = formEl.querySelector('.wpcf7-submit')
    if (!formButton) return

    const { wpcf7Id } = formEl.closest('.wpcf7')?.dataset
    ...
    document.addEventListener('wpcf7submit', function (e) {
      ...
      if (!response) return
      if (String(response.contact_form_id) !== wpcf7Id) return
      if (response.validation_passed && confirm('入力内容に問題はありません。本当に送信しますか？')) {
